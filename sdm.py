...
rootlog.title("Student Data Management  ACTIVITY LOG")
...
def done():
    ...
    if getus=="":
        ...
        con=p.connect(host="localhost",user="root",passwd=getps,database="student_management")
        if con.is_connected():
            ...
            def add():
                ...
                s=c.fetchall()
                f=s[0]
                q=f[0] if f[0] is not None else 0
                s=int(q)
                no=q+1
                adm=str(no)
